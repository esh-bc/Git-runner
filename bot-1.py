...
def repo_detail_keyboard(index: int, running: bool) -> InlineKeyboardMarkup:
    """Shows Start/Stop/Refresh/Edit/Remove/Back for a specific repo."""
    if running:
        top_row = [
    ...
    return InlineKeyboardMarkup([top_row, mid_row, back_row])
...
def edit_keyboard(index: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Change Repo URL",   callback_data=f"edit_repo_{index}")],
        [InlineKeyboardButton("🔑 Change Token",      callback_data=f"edit_token_{index}")],
        [InlineKeyboardButton("⚙️ Change Workflow",   callback_data=f"edit_wf_{index}")],
        [InlineKeyboardButton("⬅️ Back",              callback_data=f"view_{index}")]
    ])
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Edit: change repo URL ─────────────────────────────────────────────────
        ...
        if "/" not in text:
            ...
            return
        index = c.get("editing_index")
        user_repos = get_repos(user.id)
        user_repos[index]["repo"] = text
        set_repos(user.id, user_repos)
        set_conv(user.id, step=None)
        await update.message.reply_text(
            f"✅ Repo URL updated to `{text}`",
            parse_mode="Markdown",
            reply_markup=repo_detail_keyboard(index, user_repos[index].get("running", False))
        )
    ...
    # ── Edit: change token ────────────────────────────────────────────────────
        ...
        index = c.get("editing_index")
        user_repos = get_repos(user.id)
        await update.message.reply_text("⏳ Validating new token...")
        valid = await validate_github(user_repos[index]["repo"], text)
        if not valid:
            await update.message.reply_text(
                "❌ Token invalid or can't access that repo. Try again:"
            )
            return
        user_repos[index]["token"] = text
        set_repos(user.id, user_repos)
        set_conv(user.id, step=None)
        await update.message.reply_text(
            "✅ Token updated!",
    ...
    # ── Edit: change workflow filename ────────────────────────────────────────
        ...
        index = c.get("editing_index")
        user_repos = get_repos(user.id)
        user_repos[index]["start_wf"] = text
        set_repos(user.id, user_repos)
        set_conv(user.id, step=None)
        await update.message.reply_text(
            f"✅ Workflow updated to `{text}`",
            parse_mode="Markdown",
            reply_markup=repo_detail_keyboard(index, user_repos[index].get("running", False))
        )
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Edit Menu ─────────────────────────────────────────────────────────────
    elif data.startswith("edit_") and not data.startswith("edit_repo_") \
            and not data.startswith("edit_token_") and not data.startswith("edit_wf_"):
        index = int(data.split("_")[1])
        if index >= len(user_repos):
            return
        r = user_repos[index]
        await query.edit_message_text(
            ...
            parse_mode="Markdown",
            reply_markup=edit_keyboard(index)
        )

    elif data.startswith("edit_repo_"):
        index = int(data.split("edit_repo_")[1])
        set_conv(user.id, step="edit_repo", editing_index=index, temp={})
        await query.edit_message_text(
            "Send the new repository URL:\nExample: `username/repo-name`",
            parse_mode="Markdown"
        )

    elif data.startswith("edit_token_"):
        index = int(data.split("edit_token_")[1])
        set_conv(user.id, step="edit_token", editing_index=index, temp={})
        await query.edit_message_text(
            "Send the new GitHub Personal Access Token:"
        )

    elif data.startswith("edit_wf_"):
        index = int(data.split("edit_wf_")[1])
        set_conv(user.id, step="edit_wf", editing_index=index, temp={})
        await query.edit_message_text(
            "Send the new workflow filename:\nExample: `main.yml`",
            parse_mode="Markdown"
        )
